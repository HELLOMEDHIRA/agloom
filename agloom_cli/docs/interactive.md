...
The TUI maps AGP into three visible streams (same shapes custom clients should use):
...
- Stray tool-shaped JSON in the assistant stream is filtered so you see prose, not raw `{"name":"read_file",…}`.

For event payloads, examples, and UI guidance when building your own client, see **[Thinking trace & reasoning streams](../agloom/features/thinking-events.md)** (Python package docs).
...
- [Thinking trace & reasoning streams](../agloom/features/thinking-events.md) — trace vs reasoning tokens on the wire
