...
describe('token.delta', () => {
  ...
  it('accumulates reasoning role separately from assistant tokens', () => {
    dispatch({ ...env(), type: 'message.user', data: { content: 'q' } })
    dispatch({ ...env(), type: 'token.delta', data: { text: 'plan', role: 'reasoning' } })
    dispatch({ ...env(), type: 'token.delta', data: { text: 'Hi' } })
    expect(state().activeTurn?.streamedReasoning).toBe('plan')
    expect(state().activeTurn?.streamedTokens).toBe('Hi')
  })
