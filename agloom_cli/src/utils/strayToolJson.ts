 ...
 * (mirrors ``agloom.patterns.react_tool_recovery.last_ai_message_is_stray_tool_json``).
 */
...
export type StrayToolJsonOptions = {
  /** When true, strip any tool-shaped JSON even if the tool name is not in *allowedToolNames*. */
  permissive?: boolean
}

/** True when *text* is a lone JSON object describing a tool the runtime will run via structured calls. */
...
/** Strip stray tool JSON and wire-leak lines from assistant text (paragraphs and single lines). */
