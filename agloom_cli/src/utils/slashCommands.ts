...
export const SLASH_HINTS: Record<string, string> = {
  ...
  '/plan': 'Turn planner preview only (no agent run)',
  '/git': 'Harness git status or checkpoints',
  '/mcp': 'List MCP servers + tool name/description (wire catalog)',
...
export const SLASH_HELP_LINES: string[] = [
  ...
  '  /plan <goal>       Turn planner preview only',
  '  /git status        Repo summary · /git checkpoints',
  '  /mcp               MCP servers + tool names/descriptions (wire catalog)',
  ...
  '  Tab                Focus chat vs metrics scroll (split layout)',
  '  Ctrl+[/] PgUp/PgDn  Scroll the focused pane only',
