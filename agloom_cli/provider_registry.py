"""Canonical provider registry — single source of truth for the CLI's provider knowledge.

Three modules derive their tables from :data:`PROVIDERS`:

- :mod:`agloom_cli.model_resolver` — env-key snapshot, auto-detect priority order, pip-extra hints,
  ``langchain-*`` import-path checks
- :mod:`agloom_cli.provider_wizard` — wizard menu defaults (one row per ``wizard_aliases`` entry)
- :mod:`agloom_cli.llm_provider_params` — LangChain class name → canonical slug for ``describe_llm``

Add a provider once here; derived tables update automatically. Tests in
``tests/test_provider_wizard.py`` and ``tests/test_model_resolver.py`` enforce alignment.
"""
...
class ProviderInfo:
    ...
    ``slug`` is the post-:func:`agloom_cli.llm_provider_params.normalize_provider_slug` form used
