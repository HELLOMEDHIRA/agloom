...
async def handle_reflection(
    ...
    had_subtasks = bool(analysis.subtasks)
    analysis = normalize_reflection_analysis(analysis, query)
    if not had_subtasks and analysis.subtasks:
        logger.warning(
            f"[Reflection] {agent_name} — classifier returned REFLECTION with no subtasks; "
            "using user query as reflection goal."
        )
    ...
    if not analysis.subtasks:
        logger.warning(f"[Reflection] {agent_name} — no goal (empty query and no subtasks).")
