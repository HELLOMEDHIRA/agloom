...
from .wire_stream_content import split_stream_parts_from_chunk
    ...
    try:
        async with asyncio.timeout(timeout):
            ...
            async for chunk in agen:
                ...
                if reasoning:
                    payload_r: dict[str, Any] = {"content": reasoning, "role": "reasoning"}
                    if worker_id:
                        payload_r["worker_id"] = worker_id
                    await event_queue.put(AgentEvent(type="token", data=payload_r))
                if answer:
                    chunks.append(answer)
                    payload_a: dict[str, Any] = {"content": answer, "role": "assistant"}
                    if worker_id:
                        payload_a["worker_id"] = worker_id
                    await event_queue.put(AgentEvent(type="token", data=payload_a))
    finally:
