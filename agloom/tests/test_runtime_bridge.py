...
def test_translate_token_reasoning_role() -> None:
    em = capture_emitter()
    translate(AgentEvent(type="token", data={"content": "plan step", "role": "reasoning"}), em)
    assert em.calls == [
        ("emit_token_delta", {"text": "plan step", "role": "reasoning", "message_id": None}),
    ]
...
def test_translate_done_success_false_emits_error_and_returns_message() -> None:
    em = capture_emitter()
    fail_msg = translate(
        AgentEvent(
            type="done",
            data={
                "result": {
                    "pattern_used": "REACT",
                    "output": "REACT execution failed: db lock",
                    "success": False,
                    "error": "REACT execution failed: db lock",
                },
            },
        ),
        em,
    )
    assert fail_msg == "REACT execution failed: db lock"
    assert em.calls[0][0] == "emit_message_assistant"
    assert em.calls[1] == (
        "emit_error",
        {
            "severity": "fatal",
            "message": "REACT execution failed: db lock",
            "stage": "REACT",
            "retryable": False,
        },
    )
...
@pytest.mark.asyncio
async def test_bridge_closes_with_error_when_done_success_false() -> None:
    agent = _FakeAgent(
        [
            AgentEvent(
                type="done",
    ...
    assert isinstance(events[-1], SessionClosed)
    assert events[-1].data.reason == "error"
    assert "REACT execution failed" in (events[-1].data.error or "")
    assert any(e.type == "error.fatal" for e in events)


@pytest.mark.asyncio
async def test_bridge_closes_with_error_when_stream_yields_agent_error() -> None:
    agent = _FakeAgent([AgentEvent(type="error", data={"error": "provider down"})])
    buf = io.StringIO()
    await run_invocation_to_writer(agent=agent, prompt="x", writer=buf)
    events = _read_events(buf)
    assert isinstance(events[-1], SessionClosed)
    assert events[-1].data.reason == "error"
    assert events[-1].data.error == "provider down"
