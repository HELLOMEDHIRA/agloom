...
- **`astream_events()`** yields in-process **`AgentEvent`** objects (`thinking`, `classify`, `tool_call`, `token` with optional `role=reasoning`, `done`, …) for dashboards and custom UIs. See [Thinking trace & reasoning streams](../features/thinking-events.md).
