...
| **Session** | **AGP / runtime:** the bridge session id (`session` on `SessionEmitter`). **Memory:** usually the `thread_id` (and optional `user_id`) that scopes `SessionMemory` / store namespaces. |
...
| **Spawn** | One recursive invocation of a pattern via `dispatch_pattern` / `SpawnAPI`, counted against depth and budgets. |
