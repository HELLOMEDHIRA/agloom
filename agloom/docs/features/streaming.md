...
For **trace steps** vs **model reasoning tokens** (two different “thinking” streams), see **[Thinking trace & reasoning streams](thinking-events.md)**.
...
async for event in agent.astream_events("Explain gravity"):
    if event.type == "thinking":
        show_spinner(event.data.get("output", "Working…"))
    elif event.type == "classify":
        show_spinner(f"Routing · {event.data.get('pattern', '')}")
    elif event.type == "token":
        # Real-time token — fires DURING each LLM call
        role = event.data.get("role", "assistant")
        if role == "reasoning":
            show_reasoning_panel(event.data["content"])
        else:
...
- `thinking` events appear during setup (e.g. before classification); `classify` fires when routing is known
