...
| `mcp_servers` | `list[MCPServerConfig]` | `None` | MCP server connections (lazy connect on first invoke). Raises `MCPConnectionError` if connect fails or a server exposes zero tools. See [MCP Servers](../features/mcp.md) |
...
| `fallback_pattern` | `PatternType` or `None` | `None` | Advanced override for pattern routing. Leave `None` so the turn planner chooses (typical: REACT with tools, DIRECT without). Not exposed in CLI/YAML. |
| `harness` | `bool` | `False` | Inject progress + git tools. **Requires `store=` and `harness_metadata=`** when `True`. See [Harness](../features/harness.md). |
| `harness_metadata` | `HarnessMetadata` or `dict` | `None` | **Required when `harness=True`.** Fields: `project_name`, `goal`, `init_git` (default `False`), `allow_replan` (default `False`), `force_plan` (default `False`), optional `tasks` for integrator pre-seed. Forbidden when `harness=False`. |
...
| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `max_pattern_depth` | `int` | `0` | **Ceiling** for recursive pattern depth. `0` = legacy single-pass (no orchestration). When `orchestration_plan_from_classifier=True`, the turn planner picks per-turn depth ≤ this value. |
...
| `orchestration_plan_from_classifier` | `bool` | `True` | When `True` and `max_pattern_depth > 0`, the turn planner sets per-turn depth/budgets (clamped to ceilings). When `False`, agent ceilings apply directly every turn. |
...
### Turn planner fields (on `result.analysis`)

`result.analysis` is a **`QueryAnalysis`** object (alias **`TurnPlan`**). Besides pattern and subtasks:

| Field | When present | Description |
| --- | --- | --- |
| `harness_plan` | Harness on, seed/replan turn | Durable tasks persisted to the progress artifact |
| `harness_work_kind` | Harness on, seed/replan turn | Short label (`investigation`, `implementation`, …) |
| `orchestration_depth` | `max_pattern_depth > 0` | Suggested max depth for this turn (clamped) |
| `orchestration_token_budget` | Orchestration enabled | Suggested token budget |
| `orchestration_llm_call_budget` | Orchestration enabled | Suggested LLM call budget |
| `orchestration_auto_escalation` | Orchestration enabled | Suggested auto-escalation for this turn |

Legacy docs and logs may still say “classifier”; the implementation module is `agloom.src.classifier`, public entry points are **`plan_turn`** and **`analyze_query`**.
