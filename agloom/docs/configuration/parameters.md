...
| Parameter                                 | Type               | Default | Description                                                                                                                                            |
| ----------------------------------------- | ------------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `react_force_tool_choice_on_user_turn`    | `bool`             | `True`  | After each user `HumanMessage`, tool-bearing LangChain agents use `tool_choice=required` so providers that omit tools still emit a valid tool payload. Applies to the **REACT** pattern and to **workers** in SUPERVISOR, REFLECTION, SWARM, BLACKBOARD, PIPELINE, and other multi-worker patterns. Set `False` to allow prose-only first turns. |
