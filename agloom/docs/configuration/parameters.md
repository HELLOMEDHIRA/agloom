...
| Parameter                                 | Type               | Default | Description                                                                                                                                            |
| ----------------------------------------- | ------------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `react_force_tool_choice_on_user_turn`    | `bool`             | `True`  | On the **opening** user turn only, tool-bearing LangChain agents use `tool_choice=required` so providers that omit tools still emit a valid tool payload. Follow-up calls after tool results use provider defaults (avoids Qwen3/vLLM `No user query found in messages`). Applies to **REACT** and **workers** in multi-worker patterns. Set `False` to disable entirely. |
