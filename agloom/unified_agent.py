        ...
        if event_queue is not None:
            ...
            async def _stream():
                ...
                async for chunk in agent["llm"].astream(messages):
                    last_chunk = chunk
                    reasoning, answer = await emit_llm_chunk_to_event_queue(event_queue, chunk)
                    if answer:
            ...
            output = answer_text_from_content(response.content)
...
def _wire_context_preview(text: str, *, max_chars: int = _SKILL_CONTEXT_PREVIEW_MAX_CHARS) -> tuple[str, bool]:
    """Return (preview, truncated) for AGP observability fields."""
    if len(text) <= max_chars:
        return text, False
    return text[:max_chars], True


async def _build_skill_context_for_classify(
    config: dict[str, Any], *, processed_query: str
) -> tuple[str, list[str]]:
    """Skill injector + delegation targets merged for the classifier.

    Returns ``(context_text, skill_names)`` — names are manifest ids from semantic search only
    (delegation lines do not add names).
    """
    skill_ctx = ""
    ...
    if skill_injector:
        try:
            if hasattr(skill_injector, "get_context_bundle"):
                bundle = await skill_injector.get_context_bundle(processed_query)
                skill_ctx = bundle.context
    ...
    if delegation_ctx and skill_ctx:
        return f"{skill_ctx}\n\n{delegation_ctx}", skill_names
    if delegation_ctx:
        return delegation_ctx, skill_names
...
async def _emit_skill_context_event(config: dict[str, Any], skill_ctx: str, skills: list[str]) -> None:
    ...
    if eq is None:
        return
    preview, truncated = _wire_context_preview(skill_ctx)
    await eq.put(
        AgentEvent(
            type="skill_context",
            data={
    ...
    *,
    ...
    try:
        ...
        await _emit_preparation_thinking(
            eq,
            name="harness_bootstrap",
            detail=f"Harness bootstrap skipped ({exc})",
        )
    ...
    async def _try_direct_stream(
        ...
        *,
        ...
        try:
            ...
            await _emit_skill_context_event(cfg, skill_ctx, skill_names)
            augmented_query = _build_classifier_augmented_query(memory_ctx, harness_ctx, processed_query)
            analysis = await _execute_analyze_query(
                cfg,
                augmented_query=augmented_query,
        ...
        async def _translate_stream() -> None:
            ...
            try:
                async for agent_event in self.astream_events(
                    ...
                    fail_msg = translate(agent_event, emitter)
                ...
                emitter.close(reason="error", error=str(exc).strip() or repr(exc))
                agp_queue.put_nowait(None)  # sentinel — must send even on error
                return
            if invocation_failed:
                emitter.close(reason="error", error=failure_detail or "execution failed")
            else:
