...
import { Users, CheckCircle, XCircle, Loader2, Octagon } from 'lucide-react'
    ...
    <div className="h-full overflow-y-auto p-4 space-y-3">
      {workers.map((w) => (
        <div key={w.worker_id} className={cn(
          ...
          : w.status === 'halted' ? 'border-cyan-800/40 bg-cyan-950/20'
          ...
          <div className="flex items-center gap-2">
            {w.status === 'running' ? <Loader2 size={13} className="text-yellow-400 animate-spin" />
             ...
             : w.status === 'halted' ? <Octagon size={13} className="text-cyan-400" />
          ...
          {w.error && (
            <p className={cn('text-xs leading-relaxed', w.status === 'halted' ? 'text-cyan-400' : 'text-red-400')}>
              {w.error}
            </p>
          )}
