...
export const StreamingTurn = ({ turn }: Props): React.ReactElement => {
  const { userMessage, thinkingSteps, toolCalls, workers, streamedTokens, streamedReasoning, pattern } = turn
  ...
  return (
    <article className="flex flex-col gap-4">
      ...
      {(pattern || streamedReasoning || thinkingSteps.length > 0 || workers.length > 0 || toolCalls.length > 0) && (
        <div className="flex flex-col gap-1.5 pl-3 border-l-2 border-indigo-900/60 ml-1">
          ...
          {streamedReasoning && (
            <p className="text-xs text-purple-300/90 whitespace-pre-wrap leading-relaxed">
              {streamedReasoning}
            </p>
          )}
