...
const newActiveTurn = (userMessage: string): ActiveTurnState => {
  return { id: uid(), userMessage, thinkingSteps: [], toolCalls: [], workers: [], graphNodes: [], streamedTokens: '', streamedReasoning: '', pattern: null }
}
  ...
  dispatch: (evt) => set((s) => {
    ...
    switch (evt.type) {
      ...
      case 'token.delta': {
        ...
        if (role === 'reasoning') {
          return {
            ...
            activeTurn: {
              ...s.activeTurn,
              streamedReasoning: s.activeTurn.streamedReasoning + chunk,
            },
          }
        }
      ...
      case 'message.assistant': {
        ...
        const reasoningStep =
          active.streamedReasoning.trim().length > 0
            ? [
                {
                  id: uid(),
                  step: 'model_reasoning',
                  label: 'Model reasoning',
                  detail: active.streamedReasoning.trim(),
                  elapsedMs: undefined,
                },
              ]
            : []
        const turn: CompletedTurn = {
          ...
          thinkingSteps: [...reasoningStep, ...active.thinkingSteps],
      ...
      case 'skill.applied': {
        ...
        return {
          ...
          protocolNotes: pushProtocolNotes(
            s.protocolNotes,
            `Skill applied · ${evt.data.phase ?? '—'}${namePart} · +${evt.data.injected_chars ?? 0} chars${trunc}`,
          ),
        }
      }
